...
opt = optim.AdamW(wormn.parameters())
loss_func = nn.CrossEntropyLoss()

MAX_EPOCH = 10000
for i in range(MAX_EPOCH):
    opt.zero_grad()
    outp = worm_classification_forward(wormn, Xs, 2, 2, 1)/.1
...
for i in range(MAX_EPOCH):
    opt.zero_grad()
    outp = worm_classification_forward(wormn, Xs, 2, 2, 2)/.1
    cat_outp = torch.softmax(outp, axis=1)
...
for i in range(MAX_EPOCH):
    opt.zero_grad()
    outp = worm_classification_forward(wormn, Xs, 2, 2, 3)/.1
    cat_outp = torch.softmax(outp, axis=1)
...
for i in range(MAX_EPOCH):
    opt.zero_grad()
    outp = worm_classification_forward(wormn, Xs, 2, 2, 4)/.1
    cat_outp = torch.softmax(outp, axis=1)
...
for i in range(MAX_EPOCH):
    opt.zero_grad()
    outp = worm_classification_forward(wormn, Xs, 2, 2, 5)/.1
    cat_outp = torch.softmax(outp, axis=1)
...
print("++++++++++++ 0 ++++++++++++")
wormn.thresh = 8.
outp = worm_classification_forward(wormn, Xs, 2, 2, None)/.1
print(f"{outp=}")
